...
const reply = z.object({
  ...
  thread_title: z.string().trim().min(1).max(100).optional().catch(undefined),
});
      ...
      inputSchema: {
        ...
        properties: {
          ...
          thread_title: {
            type: "string",
            description:
              "A concise 2-6 word title for this conversation's actual task. Include with a substantive reply to name an untitled thread. Omit for connection checks and greetings. Existing names are preserved.",
          },
          kind: {
            type: "string",
...
mcp.setRequestHandler(CallToolRequestSchema, async (req) => {
  if (req.params.name === "acknowledge_transcript") {
    ...
    await sendReport({
      ...
      ...(a.thread_title ? { threadTitle: a.thread_title } : {}),
    });
